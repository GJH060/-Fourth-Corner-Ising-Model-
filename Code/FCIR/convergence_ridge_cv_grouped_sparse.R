...
# SPARSE data: FCIR_estimates_ridge_cv_grouped_N*_P*.Rdata
#
# Sparse truths contain exact zeros in B and A, so B/A use ABSOLUTE error while
# beta_0/alpha_0 (dense) use RELATIVE error. This matches convergence_ridge_cv_plot.R
# and differs from the dense version (convergence_ridge_cv_grouped_dense.R), where
# every parameter uses relative error.
...
output_dir = file.path(project_root, "Simulation_Results", "FCIR", "plots", "boxplots_perparam_ridge_cv_grouped_sparse")
...
for (n in Ns) {
  for (p in Ps) {
    ...
    # beta_0 and alpha_0 are dense -> relative error.
    ...
    message("Loaded grouped-CV sparse estimates: N=", n, ", P=", p)
  }
}

if (length(df_list) == 0) {
  stop("No site-grouped CV sparse estimate files were loaded.")
}
...
for (param in parameters) {
  ...
  plot_obj = ggplot(df_param, aes(x = .data$N_factor, y = .data$Error)) +
    ...
    labs(title = paste("Ridge (site-grouped CV) [SPARSE] Convergence of Parameter:", param),
         x = "Sample Size (N)",
         y = y_label) +
  ...
  plot_filename = file.path(
    output_dir,
    paste0("esterror_samplesize_ridge_cv_grouped_sparse_", safe_parameter_name(param), ".png")
  )
...
lambda_plot = ggplot(df_lambda, aes(x = .data$N_factor, y = .data$lambda)) +
  geom_boxplot(fill = "skyblue", color = "black",
               outlier.shape = 16, outlier.alpha = 0.3, width = 0.5) +
  facet_wrap(~ .data$P_label, ncol = 2) +
  # lambda spans several orders of magnitude with large outliers at small N;
  # a log10 y-axis keeps every sample size legible without a few values dominating.
  scale_y_log10() +
  theme_minimal(base_size = 14) +
  labs(title = "CV-selected Ridge Penalty (lambda.min) [SPARSE, site-grouped] across Sample Sizes",
       x = "Sample Size (N)",
       y = "Selected lambda (log10 scale)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        strip.background = element_rect(fill = "lightgray", color = NA),
        strip.text = element_text(face = "bold", size = 12))

lambda_filename = file.path(output_dir, "ridge_cv_grouped_sparse_selected_lambda.png")
ggsave(filename = lambda_filename, plot = lambda_plot, width = 12, height = 6, dpi = 300)
message("Saved plot: ", lambda_filename)

message("Finished site-grouped CV sparse per-parameter convergence plots.")
