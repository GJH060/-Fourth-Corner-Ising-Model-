...
for (n in Ns) {
  for (p in Ps) {
    ...
    data_filename = file.path(rdata_dir, paste0("FCIR_data_Dense_N", n, "_P", p, ".Rdata"))
    est_filename  = file.path(rdata_dir, paste0("FCIR_estimates_Dense_N", n, "_P", p, ".Rdata"))
    ...
    for (lambda_value in ridge_lambdas) {
      ...
      est_filename_ridge = file.path(
        rdata_dir,
        paste0("FCIR_estimates_ridge_lambda", lambda_label, "_Dense_N", n, "_P", p, ".Rdata")
      )
      
      if (!file.exists(est_filename_ridge)) {
        print(paste("Starting Ridge estimations for Dense Data N =", n, "P =", p,
                    "lambda =", lambda_value, "..."))
        est_beta_0  = matrix(NA, nrow = B_reps, ncol = L)
        est_B_mat   = array(NA, dim = c(L, K, B_reps))
        est_alpha_0 = matrix(NA, nrow = B_reps, ncol = L)
        ...
        print(paste("Ridge Dense estimates already exist for N =", n, ", P =", p,
    ...
    if (!file.exists(est_filename_ridge_cv_grouped)) {
      print(paste("Starting site-grouped CV Ridge estimations for Dense Data N =", n, "P =", p, "..."))
      est_beta_0  = matrix(NA, nrow = B_reps, ncol = L)
      est_B_mat   = array(NA, dim = c(L, K, B_reps))
      est_alpha_0 = matrix(NA, nrow = B_reps, ncol = L)
      est_A_mat   = array(NA, dim = c(L, K, B_reps))
      ...
      print(paste("Site-grouped CV Ridge Dense estimates already exist for N =", n, ", P =", p, "- Skipping."))
    }
