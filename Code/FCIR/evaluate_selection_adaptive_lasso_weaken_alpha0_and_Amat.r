...
for (n in Ns) {
  for (p in Ps) {
    f = file.path(rdata_dir, paste0("FCIR_estimates_adaptive_lasso_", init_method, infix, "_N", n, "_P", p, scenario_suffix, ".Rdata"))
    if (!file.exists(f)) {
      warning(paste("Missing:", f, "- run main_adaptive_lasso_weaken_alpha0_and_Amat.r first. Skipping."))
      next
    }
...
scenario_tag = paste0(if (use_dense) "Dense" else "Sparse", "_weaken_alpha0_and_Amat")
