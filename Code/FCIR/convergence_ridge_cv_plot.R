...
output_dir = file.path(project_root, "Simulation_Results", "FCIR", "plots", "boxplots_perparam_ridge_cv_random_sparse")
...
for (n in Ns) {
  for (p in Ps) {
    filename = file.path(
      rdata_dir,
      paste0("FCIR_estimates_ridge_cv_N", n, "_P", p, ".Rdata")
    )
    ...
    # CV-selected lambda for each replicate (unique to the ridge_cv files).
    lambda_list[[paste(n, p, sep = "_")]] = data.frame(
      N = n, P = p, lambda = env$est_lambda
    )

    message("Loaded ridge CV estimates: N=", n, ", P=", p)
  }
}
...
for (param in parameters) {
  ...
  plot_obj = ggplot(df_param, aes(x = .data$N_factor, y = .data$Error)) +
    ...
    labs(title = paste("Ridge (CV lambda.min) Convergence of Parameter:", param),
         x = "Sample Size (N)",
         y = y_label) +
  ...
  plot_filename = file.path(
    output_dir,
    paste0("esterror_samplesize_ridge_cv_", safe_parameter_name(param), ".png")
  )
...
# --- CV-specific: distribution of the selected lambda across sample sizes ---
...
lambda_plot = ggplot(df_lambda, aes(x = .data$N_factor, y = .data$lambda)) +
  ...
  facet_wrap(~ .data$P_label, ncol = 2) +
  theme_minimal(base_size = 14) +
  labs(title = "CV-selected Ridge Penalty (lambda.min) across Sample Sizes",
       x = "Sample Size (N)",
       y = "Selected lambda") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        strip.background = element_rect(fill = "lightgray", color = NA),
...
lambda_filename = file.path(output_dir, "ridge_cv_selected_lambda.png")
ggsave(filename = lambda_filename, plot = lambda_plot, width = 12, height = 6, dpi = 300)
message("Saved plot: ", lambda_filename)

message("Finished ridge CV per-parameter convergence plots.")
