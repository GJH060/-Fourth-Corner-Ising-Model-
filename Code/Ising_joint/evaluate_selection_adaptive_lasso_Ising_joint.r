...
for (n in Ns) {
  for (p in Ps) {
    f = file.path(rdata_dir,
                  paste0("Ising_estimates_adaptive_lasso", est_tag_joint, "_",
                         init_method_joint, "_N", n, "_P", p, ".Rdata"))
    if (!file.exists(f)) {
      warning(paste("Missing:", f,
                    "- run main_adaptive_lasso_Ising_joint.r first. Skipping."))
      next
    }
    ...
    message("Evaluated joint Ising adaptive lasso metrics: N=", n, ", P=", p)
  }
}

if (length(results) == 0) stop("No joint Ising adaptive lasso estimate files found.")
...
message("Saved joint Ising selection summary table: ", out_csv)
...
message("Joint Ising adaptive lasso selection table:")
writeLines(format_ascii_table(selection_wide))
message("Saved joint Ising wide selection summary table: ", selection_wide_csv)
message("Saved joint Ising bordered wide selection table: ", selection_wide_txt)
