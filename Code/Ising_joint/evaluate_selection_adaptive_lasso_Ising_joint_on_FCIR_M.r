# Evaluate joint Ising Theta against the FCIR_M true network Theta_int.
# Do not score theta_jj: FCIR_M main effects are site-varying, so there is
# no constant species-threshold truth.
...
for (n in Ns) {
  for (p in Ps) {
    f = file.path(rdata_dir,
                  paste0("Ising_estimates_adaptive_lasso", est_tag_on_fcir_m, "_",
                         init_method_joint, "_N", n, "_P", p, ".Rdata"))
    if (!file.exists(f)) {
      warning(paste("Missing:", f,
                    "- run main_adaptive_lasso_Ising_joint_on_FCIR_M.r first. Skipping."))
      next
    }
    e = new.env(); load(f, envir = e)

    ut = upper.tri(matrix(0, p, p))
    true_vec = e$Theta_int[ut]
    ...
    message("Evaluated joint Ising-on-FCIR_M Theta vs Theta_int: N=", n, ", P=", p)
  }
}

if (length(results) == 0) {
  stop("No joint Ising-on-FCIR_M adaptive lasso estimate files found.")
}
...
out_csv = file.path(out_dir, paste0("Ising_adaptive_lasso_selection",
                                    est_tag_on_fcir_m, "_",
                                    init_method_joint, ".csv"))
write.csv(df, out_csv, row.names = FALSE)
message("Saved joint Ising-on-FCIR_M selection summary table: ", out_csv)
...
selection_wide_txt = file.path(out_dir, paste0("Ising_adaptive_lasso_selection_wide",
                                               est_tag_on_fcir_m, "_",
                                               init_method_joint, ".txt"))
writeLines(format_ascii_table(selection_wide), selection_wide_txt)

message("Joint Ising-on-FCIR_M adaptive lasso selection table:")
writeLines(format_ascii_table(selection_wide))
message("Saved joint Ising-on-FCIR_M wide selection summary table: ", selection_wide_csv)
message("Saved joint Ising-on-FCIR_M bordered wide selection table: ", selection_wide_txt)
