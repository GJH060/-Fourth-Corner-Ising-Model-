...
for (n in Ns) {
  for (p in Ps) {
    ...
    est_filename = file.path(rdata_dir, paste0("FCIR_I_estimates_adaptive_lasso_", init_method, "_N", n, "_P", p, ".Rdata"))
    ...
    if (file.exists(est_filename)) {
      print(paste("FCIR_I adaptive lasso estimates already exist for N =", n, ", P =", p, "- Skipping."))
      next
    }
    ...
    if (length(bad_reps) > 0) {
      stop(paste0(
        "FCIR_I data are too class-imbalanced for binomial glmnet at N=", n, ", P=", p,
    ...
    print(paste("Starting", B_reps, "FCIR_I adaptive lasso fits for N =", n, "P =", p, "..."))
...
stopCluster(cl)
print(paste("Total FCIR_I adaptive lasso execution time:", Sys.time() - total_start))
