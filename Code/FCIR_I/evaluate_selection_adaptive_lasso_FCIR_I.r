...
for (n in Ns) {
  for (p in Ps) {
    f = file.path(rdata_dir, paste0("FCIR_I_estimates_adaptive_lasso_", init_method, "_N", n, "_P", p, ".Rdata"))
    if (!file.exists(f)) {
      warning(paste("Missing:", f, "- run main_adaptive_lasso_FCIR_I.r first. Skipping."))
      next
    }
    ...
    message("Evaluated FCIR_I adaptive lasso metrics: N=", n, ", P=", p)
  }
}

if (length(results) == 0) stop("No FCIR_I adaptive lasso estimate files found.")
...
message("Saved FCIR_I selection summary table: ", out_csv)
...
selection_wide_txt = file.path(out_dir, paste0("FCIR_I_adaptive_lasso_selection_wide_", init_method, ".txt"))
writeLines(format_ascii_table(selection_wide), selection_wide_txt)

message("FCIR_I adaptive lasso selection table:")
writeLines(format_ascii_table(selection_wide))
message("Saved FCIR_I wide selection summary table: ", selection_wide_csv)
message("Saved FCIR_I bordered wide selection table: ", selection_wide_txt)
message("Finished FCIR_I adaptive lasso selection evaluation.")
