estimate_unpenalized_FCIR_I <- function(Y, X, Tr,
    ...
    if (standardize) {
        get_intercept_cols <- seq(1, P*L, by = L)

        for(k0 in 1:ncol(glm_X)) {
            if(k0 %in% get_intercept_cols) {
                next;  # Skip standardization for species-specific intercepts
                }
            
            find_nonzeros <- which(glm_X[, k0] != 0)
            getsds[k0] <- sd(glm_X[find_nonzeros, k0]) # Only standardize using the non-zero entries
            glm_X[find_nonzeros, k0] <- glm_X[find_nonzeros, k0] / getsds[k0]
